...
const SlamLayer = () => {
  const { floorMapData, addSlamMappingData, robot_current_status, slam_frozen_map, hybirdStage, setStagePos } =
    useHybirdStore(
      useShallow((state) => ({
  ...
  const map_to_cad = grid_map ? grid_map.map_to_cad : {};
    ...
    <Group name='map'>
      ...
      {frozenImage && system_status === 2 && (
        <KonvaImage
          ...
          y={-(slamFrozenDataToCad.y + slamFrozenDataOrigin.y) * 20} // 设置图片的 y 位置
          width={slamFrozenData.width} // 设置图片的宽度
          height={slamFrozenData.height} // 设置图片的高度
          offsetX={0}
          offsetY={slamFrozenData.height}
          rotation={-(slamFrozenDataToCad.theta * 180) / Math.PI}
        />
      )}
      ...
      {addImage && [1, 2].includes(system_status) && (
        <KonvaImage
          image={addImage}
          // opacity={0.8}
          x={(addSlamBuildingToCad.x + addSlamBuildingOrigin.x) * 20} // 设置图片的 x 位置
          y={-(addSlamBuildingToCad.y + addSlamBuildingOrigin.y) * 20} // 设置图片的 y 位置
          width={addSlamBuilding.width} // 设置图片的宽度
          height={addSlamBuilding.height} // 设置图片的高度
          offsetX={0}
          offsetY={addSlamBuilding.height}
          rotation={(addSlamBuildingToCad.theta * 180) / Math.PI}
        />
      )}
    </Group>
  );
