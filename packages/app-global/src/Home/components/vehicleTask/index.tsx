          ...
          <div className='flex flex-row items-center py-2 gap-4'>
            ...
            <div className='p-2'>
              <Typography.Title level={4}>
                {t('common.home.taskId')}:{taskInfo?.task_state === 0 ? '-' : taskInfo?.task_id}
              </Typography.Title>
              <div className='flex items-center gap-6 opacity-100 text-md'>
                <div>
                  {t('common.home.taskType')}:
                  {taskInfo?.task_state === 0 ? '-' : (taskState[taskInfo?.operate_identification] ?? '-')}
                </div>
                <div>
                  {t('common.home.taskPoint')}:{taskInfo?.task_state === 0 ? '-' : taskInfo?.task_point_id}
                </div>
                <Typography.Text
                  style={{
