...
export const rcsLanguageDict: Record<string, any> = {
  zh_CN: 'zh-Hans',
  en_US: 'en',
  ja_JP: 'ja',
  ko_KR: 'ko',
  ...
  useEffect(() => {
    ...
    connection.on('StorageStateMessage', async (data) => {
      ...
      setWssLocationStateHashMap({
        ...
        [newData.pointId]: {
          state: newData.state,
          locationCode: newData.locationCode,
        },
      });
    });
    ...
    connection.on('GetVehicleStateMessage', async (data) => {
      // console.log('VehicleLocationMessage', data);
      // updateVehicles(data);
    });
