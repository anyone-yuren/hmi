...
const Sidebar = () => {
  ...
  const updateNodeTemplate = useWorkflowStore((state) => state.updateNodeTemplate);
  const deleteNodeTemplate = useWorkflowStore((state) => state.deleteNodeTemplate);
  ...
  const handleDelete = (index: number) => {
    Modal.confirm({
      ...
      onConfirm: () => deleteNodeTemplate(index),
    });
  };
    ...
    <div className="flex h-full w-60 flex-col border-r border-gray-700 bg-[#1f1f1f] text-gray-200">
      <div className="flex items-center justify-between border-b border-gray-700 p-4">
        ...
        <Tooltip title="添加新节点模板">
          <Button type="text" icon={<PlusOutlined className="text-gray-300" />} onClick={handleAdd} />
        </Tooltip>
      </div>
            ...
            >
              ...
              <div className="absolute right-1 top-1 hidden gap-1 group-hover:flex">
                <Button 
                  type="text" 
                  size="small" 
                  icon={<EditOutlined className="text-xs text-gray-400 hover:text-blue-400" />} 
                  onClick={(e) => { e.preventDefault(); handleEdit(index, node); }}
                />
                 <Button 
                  type="text" 
                  size="small" 
                  icon={<DeleteOutlined className="text-xs text-gray-400 hover:text-red-400" />} 
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="icon" label="图标">
            <Select options={Object.keys(iconMap).map(key => ({ value: key, label: key }))} />
          </Form.Item>
