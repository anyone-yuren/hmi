...
const BaseNode = memo(
  ({
    ...
    return (
      <div
        className={clsx(
          'min-w-[200px] rounded-lg border-2 bg-[#2a2a2a] shadow-md transition-all group',
          selected
      ...
      >
        ...
        <div
          className={clsx(
            ...
            `bg-${color}-900/30 border-b border-gray-700`,
          )}
        >
        ...
        <div className='p-3 text-sm text-gray-400'>
          {data.description && (
            <div className='mb-2 text-xs text-gray-500'>{data.description}</div>
          )}
          {children}
        </div>
        ...
        {!isEnd && (
          <div className='absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 z-10'>
            <div
              className='relative flex items-center justify-center w-3 h-3 bg-gray-400 rounded-full hover:bg-blue-500 cursor-pointer transition-colors'
              onClick={(e) => {
                e.stopPropagation();
                handleConnectClick(e, 'source');
              }}
            >
              <PlusOutlined className='text-white text-[8px]' />
              <Handle
                type='source'
                position={Position.Right}
                className='!w-full !h-full !opacity-0 !border-none !bg-transparent absolute inset-0'
              />
            </div>
          </div>
        )}
