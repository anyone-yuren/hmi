...
const TaskAction = forwardRef((props: any, ref) => {
  ...
  const { loading: updateTemplateLoading, runAsync: updateTemplateAsync } =
    useRequest((params) => updateTemplate(params), {
      manual: true,
      onSuccess: (res: any) => {
  ...
  useEffect(() => {
    console.log("preTaskList", preTaskList);
  }, [preTaskList]);
  ...
  const validateParams = (params: any) => {
    ...
    for (let index = 0; index < params.tasks.length; index++) {
      (!params.tasks[index]["task_type"] ||
        !params.tasks[index]["task_point_id"]) &&
        (isPass = false);
    }
    return isPass;
  };
                      ...
                      {task.expand && (
                        <div className="flex gap-[5px] pb-[5px]">
                          <InputGroup sx={{ flex: 1 }}>
                            <div className="title">
                              {t("deployer.singleTask.pallet")}
                            </div>
                            <MapTaskSelect
                              variant={"outlined"}
                              displayEmpty
                              value={task?.pallet_id}
                              onChange={(event: any) => {
                                const name = event.target.value;
            ...
            <div style={{ flex: 2 }}>
              <Button
                ...
                onClick={async () => {
                  const params = transformTaskListToParams(
                    {
                      loop_count: loopTime,
                      task_interval: intervalTime,
                      tasks: preTaskList.filter(
                        (item: any) => !!item?.task_type,
                      ),
                    },
                    isKVehicle,
                  );
                  ...
                  const sendParams = {
                    old_info_name: initTemplateName,
                    new_template_task: templateParams,
                  };
  ...
  return (
    ...
    >
      ...
      <div className="content" style={{ height: "24px" }}>
        <div style={{ fontSize: 16, overflow: "scroll", whiteSpace: "nowrap" }}>
          {value}
        </div>
