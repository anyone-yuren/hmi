...
const ArrowIconBox = (props: any) => {
  return (
    <div
      {...props}
      className="flex-1 flex items-center justify-items-center bg-[#627881] rounded-[6px]"
    >
      {props.children}
    </div>
  );
...
const RcsTaskPanel = (props: IProps) => {
      ...
      onSuccess: (response) => {
        const ary =
          response?.data?.items?.map((item: any) => {
            ...
            const palletAry = JSON.parse(
              template?.extraProperties.containerTypeCodes,
            );
            return {
              id: item.id,
              ...
              tasks: template?.missionItems?.map((item) => ({
                ...
                task_type:
                  rcsConfigHashMap[item.missionItemType]?.type || "Null",
  ...
  const list = useMemo(() => {
    return (
      { task: taskList || [], template: templateTaskList || [] }[activeKey] ||
      []
    );
    ...
    return {
      ...
      Pick: {
        translateToRcsParams: (obj) => {
          ...
          obj["extraProperties"] = {};
          obj["param"][0] &&
            (obj["extraProperties"]["InitialHeight"] = obj["param"][0]);
          obj["param"][1] &&
            (obj["extraProperties"]["FinalHeight"] = obj["param"][1]);
          obj["param"][2] &&
            (obj["extraProperties"]["Param3"] = obj["param"][2]);
          obj["param"][3] &&
            (obj["extraProperties"]["Param4"] = obj["param"][3]);
  ...
  const getParamsFormTaskList = (ary) => {
    ...
    for (let index = 0; index < ary.length; index++) {
      ...
      types.push(type);
      missionItems.push(
        rcsConfigHashMap[type].translateToRcsParams(ary[index]),
      );
    }
    types.includes("Charge")
      ? (initParams["missionType"] = 1)
      : (initParams["missionType"] = 0);
          ...
          return (
            <TaskItem>
              ...
              >
                <div className="title">
                  <span>
                    {t("deployer.singleTask.trajectory")}:
                    {taskItem?.trajectory || "-"}
                  </span>
                </div>
                <div className="content">
        ...
        <MapTaskPanelAction>
          ...
          <div className="listContainer">
            {preTaskList.map((task: any, index: number) => {
              return (
                <div
                  className="bg-[#00d1d11a] flex rounded-[10px]"
                  key={"task_action_" + task?.id}
                >
                  ...
                  <div className="flex flex-col flex-1">
                      ...
                      <div className="flex-1">
                        <PointOrLineBox
                          ...
                          list={[
                            ...
                            {
                              id: "Place",
                              label: t("common.taskState.pickDown"),
                            },
                            {
                              id: "Charge",
                              label: t("common.taskState.charging"),
                            },
                    ...
                    {task.task_type === "Charge" && (
                      <div className="flex gap-[6px] m-[6px] mt-[0px]">
                        <InputGroup sx={{ flex: 1 }}>
                          <div className="title">
                            {t("deployer.singleTask.chargeType")}
                          </div>
                          <MapTaskSelect
                            variant={"outlined"}
                            displayEmpty
                          ...
                          >
                            <MenuItem value={1}>
                              {t("deployer.singleTask.percentage")}
                            </MenuItem>
                            <MenuItem value={3}>
                              {t("deployer.singleTask.time")}
                            </MenuItem>
                          </MapTaskSelect>
                        </InputGroup>
                        <InputAntdText
                          title={
                            t("deployer.singleTask.threshold") +
                            (task.task_charge_type === 1 ? "(%)" : "(h)")
                          }
