...
function CameraController({ lookPosition, controlsRef, floorSize }) {
  ...
  useEffect(() => {
    if (targetRef.current) {
      targetRef.current.position.set(lookPosition.x / 1000, 0, -lookPosition.y / 1000);
    }
  }, [lookPosition]);
  ...
  // 🚀 动态设置 shadow camera 范围
  useEffect(() => {
    if (lightRef.current && floorSize) {
      const halfWidth = floorSize.width / 2;
      const halfHeight = floorSize.height / 2;
  ...
  return (
    <group>
      ...
      <PerspectiveCamera
        // ref={cameraRef}
        makeDefault
        position={[lookPosition.x / 1000, 20, 0 - lookPosition.y / 1000]}
        fov={75}
      ...
      <CameraController
        ...
        floorSize={{
          width: convertToMeters(mapSize.width),
          height: convertToMeters(mapSize.height),
        }}
      />
