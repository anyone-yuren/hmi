...
import { BoxGeometry, MeshStandardMaterial, Spherical, Vector3 } from 'three';
  ...
  return (
    <>
      {mapVertices.map((item) => {
        const position = [convertToMeters(item.x), 0.8, convertToMeters(0 - item.y)];
        return (
          <>
            <group key={item.pointId} position={position}>
              {/* <mesh geometry={geometry} material={material} /> */}
              <Suspense>
                <Svg src='/icons/point.svg' scale={0.04} position={[-0.4, 0, 0]}></Svg>
                <LineText
                  edgeId={item.pointId}
                  midPoint={[0, 0.01, 0]}
                  directionType={1}
