  ...
  return (
    <Suspense>
      <animated.group position={groupProps.position as unknown as THREE.Vector3} rotation={[0, 0 - rotationRadians, 0]}>
        ...
        {isHasGoods ? (
          <mesh geometry={baseMesh.geometry} position={[0, 0.6, 0.3]} material={material} castShadow receiveShadow />
        ) : null}
        <DeviceItem
          modelPaths={(import.meta as any).env.MODE === 'development' ? ['/car/Mxw-1.FBX'] : ['/twin/car/Mxw-1.FBX']}
        ...
        <Html distanceFactor={20} position={[0, 3, 0]} center>
          <BorderBox10 color={['#0d9488', '#ffffff00']} dur={6}>
            <Card
              ...
              styles={{
                ...
                header: {
                  padding: '0 6px',
                  minHeight: '32px',
                  borderBottom: '1px solid #079586',
                  color: 'white',
                },
              }}
            >
              <div className='flex flex-col gap-2 divide-y divide-gray-100 divide-opacity-20 text-white'>
                {items.map((item) => {
                  return (
